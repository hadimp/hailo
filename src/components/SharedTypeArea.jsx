...
window.SharedTypeArea = ({ title, text, targetSentence, isActive, success, theme }) => {
    ...
    return (
        <div className={`w-full ${isMobile ? 'pt-0' : 'pt-14 md:pt-16'} px-2 sm:px-4 md:px-8 flex justify-center items-start z-30 shrink-0`}>
            <div className={`glass-panel px-3 sm:px-4 ${isMobile ? 'py-1.5' : 'py-3 sm:py-4'} w-full max-w-md text-center transition-all duration-300 ${isActive ? `${currentTheme.focusPanel} border-opacity-50` : 'shadow-lg border-slate-700/30'}`}>
                <h2 className={`text-[9px] sm:text-[10px] font-bold tracking-[0.2em] uppercase ${isMobile ? 'mb-1' : 'mb-2 sm:mb-4'} transition-colors ${isActive ? currentTheme.text : 'text-slate-500'}`}>
                    TYPE THIS TEXT:
                </h2>
                {targetSentence && (
                ...
                <div className={`text-2xl lg:text-4xl font-light text-white tracking-wide break-words ${isMobile ? 'min-h-[30px]' : 'min-h-[40px]'} flex items-center justify-center`}>
